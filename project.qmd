...
Through the semester we've spent a lot of time digging into technical details of computational methods. In the semester project, however, we take a step back to see the forest from the trees. How does one actually think about **data** and the questions that the dataset can help answer? Your project may turn out to be very descriptive in nature (in which case your visuals should be technically saavy). Your project my turn out to use some of the machine learning methods we've learned in class -- are there modifications to the ML method that are needed because of the nuances of your particular dataset?  Here is your chance to synthesize the ideas from class and think about how you can take what you've learned into a real world problem.
...
2. Your project must tell us something. We've seen a range of projects / analyses over the course of the semester. On one extreme are visualization projects (like Snow's cholera map) that involve virtually no statistical analysis. On the other extreme are data mining projects like the KDD cup or Kaggle competitions, which involve very little visualization. Your project can be anywhere on this spectrum, but expectations may be different depending on where you are on the scale. An example of a project that doesn't tell us anything, would be something that downloads a single data source and summarizes it, with some perfunctory visualization. Make sure that your project is thought-provoking and has some underlying meaning!
   * You could do a project that involves <a href = "https://closeread.dev/" target = "_blank">Data Scrollytelling</a> ... and you could submit your work to the <a href = "https://www.causeweb.org/cause/contests/data-scrollytelling" target = "_blank">Data Scrollytelling Contest</a> Deadline Friday, December 19, 2025!
   * You might also consider submitting your project to the <a href = "https://www.causeweb.org/usproc/" target = "_blank">Undergraduate Statistics Project Competition</a> Deadline Thursday, December 18, 2025.

3. To get an A, you must show something beyond what we've done in class but that you don't already know. Some examples:
Use a new data mining technique (e.g., an artificial neural network, näive Bayes classifier -- only if the technique is a *new* idea for you!)
Use a new programming technology (e.g., Python, MapReduce, shell scripting...  should be *new* to you!)  
   * Use a new visualization technology (e.g., Scrollytelling, Shiny)  
   ...
   * Engage with a new data format (e.g., XML, HDF5, JSON)  
...
Your initial proposal (to Canvas) is due by midnight on Friday, October 17th. The final proposal (to GitHub project site) will be due two weeks later on October 29th.
...
Your initial (to Canvas) and revised (to GitHub) proposals should contain the following content:
...
Please respond to the following three questions (posted to your GitHub repo) by the end of the day Friday, November 14th.
...
2. Ideally, you will include a plot or table or some kind of exploratory data analysis indicating that you have the data and understand how to work with it.

3. What is the single biggest unresolved issue you are having? Please describe it briefly, and what your plan is for resolving this issue.

4. What are the elements from outside of the course, if any, that you plan to incorporate into your project?
...
* The project must have substantial elements from what we did in class.
* The work must be completely reproducible (either a single Quarto file or instructions on how to reproduce the work using other files).
* All work must be done in R unless you clear use of other software in advance. Doing the entire project in Python will lead to a substantially lower grade. I like Python, really! But the focus in our class is honing your R skills.
* The introduction should start with some exploratory data analysis.  Introduce the reader to the data, explain the problem, show some graphs.
* Graphs should include descriptive labels and captions.
 
The report should be a write-up.  Not a homework assignment where you answer the rubric’s questions.  It should be a full analysis of the dataset and explanation of the project.
...
Your write-up will be similar to a report to client crafted in Quarto, compiled to a pdf, due on Monday, December 8th by midnight. 
...
6. What ethical considerations came about during the project?  How were you or weren't you able to address those issues?  
7. What did you learn beyond what we've covered in class?

### Style

The notebook (i.e., qmd + pdf) format is designed to be an interactive document. Take advantage of this by including hyperlinks, figures, videos, etc. to provide context for the reader. It is likely that you will include references section or a <a href = "http://rmarkdown.rstudio.com/authoring_bibliographies_and_citations.html" target = "_blank">bibliography</a>, and your references should be embedded via links. Use Quarto elements like links, lists, LaTeX, and images as needed.
...
(Projects completed as a Math 154 assignment are denoted with <i class="fa-solid fa-award"></i>)

* <i class="fa-solid fa-award"></i> <a href = "https://ziqixiong.shinyapps.io/TopicModeling" target = "_blank">Topic Modeling</a> by Benji Lu, Kai Fukutaki, and Ziqi Xiong
...
* <i class="fa-solid fa-award"></i> <a href = "https://tcai.shinyapps.io/airbnbtextanalysis/" target = "_blank">Airbnb Text Analysis</a> by Tianyuan Cai, Jiwon Lee, and Ethan Matlin
* <i class="fa-solid fa-award"></i> <a href = "https://github.com/robinspollak/R.openFEC" target = "_blank">R package for downloading FEC data</a> by Robin Pollak and Jeff Carney 
* <i class="fa-solid fa-award"></i> <a href = "https://math154-fall2017-vegas.shinyapps.io/lvmapbackup/" target = "_blank">Vegas Baby</a> by Gwen Song, Amy Oden, and Judy Cheng
* <i class="fa-solid fa-award"></i> <a href = "https://fordc5.github.io/ExtremeWeatherAndTerrorism/" target = "_blank">Weather and Terrorism</a> by Connor Ford and Bradley Druzinsky
* <i class="fa-solid fa-award"></i> <a href = "https://mweinstock02014.shinyapps.io/twitter_app/" target = "_blank">NGO information via Twitter</a> by Molly Weinstock, Alex Pusch, and Melissa Hooke 
...
* <i class="fa-solid fa-award"></i> <a href = "https://bwhqci-kae999.shinyapps.io/groupa-music/" target = "_blank">PoemR</a>, a Shiny app to generate poems from song lyrics by Brian Bishop, Kamil English, and Danny Ta. 
* <i class="fa-solid fa-award"></i> <a href = "https://mterdiman.shinyapps.io/shinycrime/" target = "_blank">Crime and Soccer</a>, a Shiny app to explore trends in crime (specifically relating to violence/sexual violence) around two major Premier League soccer stadiums. by Mira Terdiman and Hubery Hu.
