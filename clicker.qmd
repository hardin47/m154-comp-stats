...
82. Simplifying the decision tree by pruning peripheral branches will cause over fitting.
...
 
83. All are true with regards to PRUNING except:
...
84.  Regression trees are invariant to monotonic transformations of:
...
86. bagging uses bootstrapping on:
...
 
88.  oob samples are great because
...
 
89.  bagging is random forests with:
...
 
80 training observations, 20 test observations
...
 
90.  With random forests, the value for m is chosen
...
92.  With binary response and X1 and X2 continuous, kNN (k=1) creates a linear decision boundary.
...
93. With binary response and X1 and X2 continuous, a classification tree with one split creates a linear decision boundary.
...
 
94.  With binary response and X1 and X2 continuous, a classification tree with one split creates the best linear decision boundary.
