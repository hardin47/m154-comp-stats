...
39. Consider the `addTen()` function.  The following output is a result of which `map_*()` call?[^39]
...
40. Which of the following input is allowed?[^40]
    a. `map(c(1, 4, 7), addTen)`
...
41. Which of the following produces a different output?[^41]
    a. `map(c(1, 4, 7), addTen)`
...
42. What will the following code output?[^42]
    a. 3 random normals
    b. 6 random normals
    c. 18 random normals
