...
## Aside: a few important R functions (`ifelse()`)
```{r}
data.frame(value = c(-2:2)) %>%
  mutate(abs_value = ifelse(value >=0, value, -value))  # absolute value
```

---

## Aside: a few important R functions (`case_when()`)
```{r}
set.seed(4747)
...
## Aside: a few important R functions (`set.seed()`)
...
## Aside: a few important R functions (and/or)
    ```{r}
diamonds %>% 
