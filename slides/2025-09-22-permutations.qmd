...
```{r out.width='100%', fig.align="center",  echo=FALSE}
#| fig.cap: "Gender bias seen in study on 20,000 students. The students knew the gender of their prof, but did not choose their class. https://doi.org/10.1093/jeea/jvx057"
...
```{r out.width='100%', fig.align="center", echo=FALSE}
#| fig.cap: "86 students assigned randomly to the TA section. *Innovative Higher Education*, **40**, pages 291–303 (2015). https://doi.org/10.58188/1941-8043.1509"
...
#| fig.cap: "Analysis was done using t-tests. https://doi.org/10.58188/1941-8043.1509"
...
5. Create a null sampling distribution of the $P$ copies of $b^*_{1\cdot 2}$.

6. Compare the observed test statistic to the permuted null sampling distribution from step 5.
...
3. Fit a model on the permuted $Y^*$ values (regress $Y^*$ on both $X_1$ and $X_2$) to obtain permuted coefficient estimate: $b^*_{1\cdot2}$ 
$$\widehat{Y}^* = b^*_{0\cdot1,2} + b^*_{1\cdot2}X_1 + b^*_{2\cdot1}X_2$$
...
* small takeaway -- it turns out that even when both the normality conditions and the exchangeability are modestly violated, all three of the permutation test methods described below do reasonably well at controlling Type I errors [@AndersonLegendre99;@Winkler2014].
