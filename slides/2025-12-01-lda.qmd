...
ggplot() +
  geom_text(data = words, 
            aes(x = x2, y = y2, 
                label = words2, 
                color = factor(topic2, levels = c("T3", "T2", "T1"))), 
            size = 8) +
  coord_fixed() +  
  theme_void() + 
  theme(legend.position = "none")
...
* $\alpha$ controls the document-topic density
   - small $\alpha$ is used if documents are sparse in topics $\rightarrow$ puts most probability on only a few topics
   - large $\alpha$ is used if documents mix many topics $\rightarrow$ spreads probability across many topics
   
* $\beta$ controls the topic-word density
   - small $\beta$ means topics are sparse, with few high-probability words $\rightarrow$ puts most probability on a few words
   - large $\beta$ means topics are smoother, lots of words with small but non-zero probability $\rightarrow$ spreads probability across more words
...
where $\theta_{d,k}$ is the probability of topic $k$ for document $d$ and estimated as the proportion of topic $k$ in document $d$.
