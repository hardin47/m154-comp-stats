...
When simulating, it's important to understand the difference between *sampling*, *shuffling*,  and *resampling*.  We'll use the function `sample()` to do all three:
...
sample(alph, 20, replace = TRUE) # sample (from a population)

sample(alph, replace = FALSE)  # shuffle

sample(alph, replace = TRUE)  # resampling
```
...
```{r}
set.seed(4747)
runif(4) # random uniform numbers
...
* Generate new data and use the two models above. 

* How well do the models accurately predict if the student has `talent` > 115?
...
* depending on the nature of the distribution difference, this can be either to the benefit or the detriment of the minority population
...
See: http://www.rossmanchance.com/applets/RegShuffle.htm
